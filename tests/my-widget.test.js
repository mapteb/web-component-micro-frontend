...
describe('MyWidget connectedCallback', () => {
  ...
  test('should not affect other event types', () => {
    // Arrange
    // Create a fresh component instance
    component.connectedCallback();
    const initialHTML = component.innerHTML;

    // Act
    const otherEvent = new CustomEvent('other-event', {
      detail: { message: 'Should not affect component' }
    });
    document.dispatchEvent(otherEvent);

    // Assert
    expect(component.innerHTML).toBe(initialHTML);
  });
  
  test('should work when component is added to DOM', () => {
    // Arrange
    document.body.appendChild(component);
    ...
    component.connectedCallback();
    const customEvent = new CustomEvent('my-custom-event', {
      detail: { message: 'DOM test message' }
    });
    document.dispatchEvent(customEvent);

    // Assert
    expect(component.innerHTML).toContain('<p>Received: DOM test message</p>');
  });  

});


// Integration test with actual DOM
describe('MyWidget Integration Tests', () => {
  let component;
  ...
  test('should handle events after being added to DOM', () => {
    // Arrange
    document.body.appendChild(component);

    // Act
    const customEvent = new CustomEvent('my-custom-event', {
      detail: { message: 'Integration test message' }
    });
    document.dispatchEvent(customEvent);

    // Assert
    expect(component.innerHTML).toContain('<p>Received: Integration test message</p>');
  });
});